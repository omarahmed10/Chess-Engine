...
public abstract class Piece {
	...
	public Piece(Point initialPosition, int armyType, Object[][] board) {
		if (armyType != WHITE_ARMY && armyType != BLACK_ARMY) {
			throw new RuntimeException("Error , 0 for white & 1 for black");
		}

		if (isOutOfBounds(initialPosition)) {
			throw new RuntimeException("The point is out of the board's bounds");
		}
	...
	public boolean move(Point toPosition, Object[][] board, List<Piece> graveyard) {
		if (!isDead()) {
			
			if (availablePositions.contains(toPosition)) {
				
				// if this position has an enemy , then we send it to the graveyard
				if (getSquareStatus(toPosition, board) == HAS_ENEMY) {
					((Piece) board[toPosition.x][toPosition.y]).sendToGraveyard(graveyard);
				}
	...
	public boolean isOutOfBounds(Point testedPoint) {
		...
		return (testedPoint.x >= BOARD_HIGH_LIMIT || testedPoint.x < BOARD_LOW_LIMIT)
				|| (testedPoint.y >= BOARD_HIGH_LIMIT || testedPoint.y < BOARD_LOW_LIMIT);
	}
